...
struct PublicProfileModel: Equatable, Sendable, Identifiable {
    ...
    var submissionCountLabel: String {
        ...
        return "\(submissionCount) sketches"
    }
}

protocol ProfileFetching: Sendable {
    func fetchPublicProfile(username: String, accessToken: String?) async throws -> PublicProfileModel
...
struct ProfileRepository: ProfileFetching {
    ...
    func fetchPublicProfile(username: String, accessToken: String?) async throws -> PublicProfileModel {
        ...
        do {
            let user = try await UsersAPI.getPublicUser(username: username)
