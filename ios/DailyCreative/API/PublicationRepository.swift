...
enum PublicationContent: Sendable {
    case sketch(uploadId: UUID, caption: String?)
    case story(body: String, caption: String?)
}

protocol PublicationServing: Sendable {
...
typealias SubmissionServing = PublicationServing
typealias SubmissionRepository = PublicationRepository
typealias RecordingSubmissionRepository = RecordingPublicationRepository
...
struct PublicationRepository: PublicationServing {
    let baseURL: URL
    ...
    func createPublication(
        ...
        guard let apiCreativeType = CreativeType(rawValue: creativeType) else {
            throw PublicationAPIError.underlying("Unsupported creative type.")
        }
        configureClient(accessToken: accessToken)
        do {
            let contentPayload: CreateSubmissionRequestContent
            switch content {
            case .sketch(let uploadId, let caption):
                contentPayload = .typeSketchSubmissionContent(
            ...
            case .story(let body, let caption):
                contentPayload = .typeStorySubmissionContent(
    ...
    private func mapSubmission(_ submission: Submission) -> SubmissionModel {
        SubmissionModel(
            ...
            sketchSessionId: submission.creativeType == .sketch ? submission.sessionId : nil,
            storySessionId: submission.creativeType == .story ? submission.sessionId : nil,
...
final class RecordingPublicationRepository: PublicationServing, @unchecked Sendable {
    private(set) var createCallCount = 0
