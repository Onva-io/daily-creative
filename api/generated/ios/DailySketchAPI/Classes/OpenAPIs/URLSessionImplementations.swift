...
public typealias DailySketchAPIAPIChallengeHandler = ((URLSession, URLSessionTask, URLAuthenticationChallenge) -> (URLSession.AuthChallengeDisposition, URLCredential?))
...
private var challengeHandlerStore = SynchronizedDictionary<Int, DailySketchAPIAPIChallengeHandler>()
...
open class URLSessionRequestBuilder<T>: RequestBuilder<T> {
     ...
     */
    public var taskDidReceiveChallenge: DailySketchAPIAPIChallengeHandler?
    ...
    @discardableResult
    override open func execute(_ apiResponseQueue: DispatchQueue = DailySketchAPIAPI.apiResponseQueue, _ completion: @escaping (_ result: Swift.Result<Response<T>, ErrorResponse>) -> Void) -> RequestTask {
    ...
    open func buildHeaders() -> [String: String] {
        ...
        for (key, value) in DailySketchAPIAPI.customHeaders {
        ...
        case is URL.Type:
            do {
                ...
                if let headerFileName = getFileName(fromContentDisposition: httpResponse.allHeaderFields["Content-Disposition"] as? String) {
                    ...
                    requestPath = requestPath.appending("/tmp.DailySketchAPI.\(UUID().uuidString)")
                }
